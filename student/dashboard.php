<?php 
include('header.php');
if(!isset($_GET["sid"]) || !isset($_SESSION["token"])){
  session_unset();
  session_destroy();
  header('location:../');
  exit();
...
$imagePath = "../img/pictures/{$decryptedId}.jpg";
$defaultImage = "../img/pictures/default.jpg";
if (file_exists($imagePath)) { $imgSrc = $imagePath;} 
else { $imgSrc = $defaultImage; }
?>
<style>.bold{font-weight:bold;color:#011;}</style>
<div class="text-start" style="max-width: 280px;">
    <div class="text-center">
      <img class="mb-1" src="<?=htmlspecialchars($imgSrc)?>" alt="Student Profile" style="width:150px;border:2px solid #DFDFDF;border-radius:8px;">
    </div>
    <hr>
